-- ==========================================
-- DRMS - Partner/Merchant Full Schema (PG)
-- ==========================================
...
-- =========================
CREATE TABLE IF NOT EXISTS tb_partner_account (
                                                  id           BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS tb_merchant_account (
                                                   id           BIGSERIAL PRIMARY KEY,
                                                   role_id      BIGINT NOT NULL REFERENCES tb_role(id),
    email        VARCHAR(255) NOT NULL UNIQUE,
    password     TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS tb_partner_info (
                                               id                 BIGSERIAL PRIMARY KEY,
                                               partner_account_id BIGINT NOT NULL REFERENCES tb_partner_account(id) ON DELETE CASCADE,
    ...
    created_date       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_date       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_partner_info_account UNIQUE (partner_account_id)
    );

CREATE TABLE IF NOT EXISTS tb_merchant_info (
                                                id                   BIGSERIAL PRIMARY KEY,
                                                merchant_account_id  BIGINT NOT NULL REFERENCES tb_merchant_account(id) ON DELETE CASCADE,
    first_name           VARCHAR(150),
    last_name            VARCHAR(150),
    gender               VARCHAR(20),
    address              TEXT,
    ...
    created_date         TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_date         TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_merchant_info_account UNIQUE (merchant_account_id)
    );

CREATE TABLE IF NOT EXISTS tb_partner_phone (
                                                id              BIGSERIAL PRIMARY KEY,
                                                partner_info_id BIGINT NOT NULL REFERENCES tb_partner_info(id) ON DELETE CASCADE,
    phone_number    VARCHAR(50) NOT NULL
    );

CREATE TABLE IF NOT EXISTS tb_merchant_phone (
                                                 id                BIGSERIAL PRIMARY KEY,
                                                 merchant_info_id  BIGINT NOT NULL REFERENCES tb_merchant_info(id) ON DELETE CASCADE,
    phone_number      VARCHAR(50) NOT NULL
    );
...
CREATE TABLE IF NOT EXISTS tb_rating_detail (
    ...
    comment             TEXT,
    created_date        TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_rating_store_merchant UNIQUE (store_id, merchant_account_id)
    );
...
CREATE TABLE IF NOT EXISTS tb_bookmark (
    ...
    merchant_account_id BIGINT NOT NULL REFERENCES tb_merchant_account(id) ON DELETE CASCADE,
    created_date        TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_bookmark_store_merchant UNIQUE (store_id, merchant_account_id)
    );
...
CREATE TABLE IF NOT EXISTS tb_partner_notification (
                                                       id               BIGSERIAL PRIMARY KEY,
                                                       partner_account_id BIGINT NOT NULL REFERENCES tb_partner_account(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS tb_merchant_notification (
                                                        id                  BIGSERIAL PRIMARY KEY,
                                                        merchant_account_id BIGINT NOT NULL REFERENCES tb_merchant_account(id) ON DELETE CASCADE,
    type_id             BIGINT NOT NULL REFERENCES tb_notification_type(id),
    content             TEXT,

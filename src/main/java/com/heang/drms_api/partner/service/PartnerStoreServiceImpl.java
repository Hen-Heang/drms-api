...
public class PartnerStoreServiceImpl implements PartnerStoreService {
    ...
    public Boolean checkStoreIfCreated(Integer currentUserId) {
        int check = 2;
        check = partnerStoreMapper.checkStoreIfCreated(currentUserId);
        return check == 1;
    }

    Boolean checkDuplicateStoreName(String name) {
        return partnerStoreMapper.checkDuplicateStoreName(name);
    }
    ...
    public Store createNewStore(StoreRequest storeRequest, Integer currentUserId) throws ParseException {
        ...
        AppUser appUser = authUserMapper.findPartnerUserById(currentUserId);
        ...
        if (appUser.getIsVerified()) {
            ...
            store = partnerStoreMapper.createNewStore(storeRequest, currentUserId);
            for (String phone : storeRequest.getAdditionalPhone()) {
                if (phone.isBlank()){
                    continue;
                }
        ...
        Store newStore = partnerStoreMapper.getUserStore(currentUserId);
    ...
    @Override
    public Store getUserStore(Integer currentUserId) throws ParseException, NotFoundException {
        // get store for current user
        Store store = partnerStoreMapper.getUserStore(currentUserId);
    ...
    public Store editAllFieldUserStore(Integer currentUserId, StoreRequest storeRequest) throws ParseException {
        ...
        Integer storeId = partnerStoreMapper.getStoreIdByUserId(currentUserId);
        ...
        Store store = partnerStoreMapper.editAllFieldUserStore(storeId, storeRequest);
        // delete additional phone
        partnerStoreMapper.deleteAdditionalPhone(storeId);
        for (String phone : storeRequest.getAdditionalPhone()) {
            if (phone.isBlank()){
                continue;
            }
    ...
    public String deleteUserStore(Integer currentUserId) {
        ...
        String store = partnerStoreMapper.deleteUserStore(currentUserId);
        if (store == null) {
    ...
    public String disableStore(Integer currentUserId) {
        ...
        if (!partnerStoreMapper.checkIfStoreIsDisable(currentUserId)) {
            throw new ConflictException("Store is already disabled.");
        }
        String store = partnerStoreMapper.disableStore(currentUserId);
        if (store == null) {
            throw new InternalServerErrorException("Something went wrong while deleting");
        }
    ...
    public String enableStore(Integer currentUserId) {
        ...
        String store = partnerStoreMapper.enableStore(currentUserId);
        if (store == null) {
            throw new InternalServerErrorException("Something went wrong while deleting");
        }
